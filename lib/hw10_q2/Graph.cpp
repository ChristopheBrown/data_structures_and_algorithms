...
//  DList unvisitedQueue;
  ...
  for (int i = 0; i < numNodes; i++)
    unvisitedQueue.insert(i);

  while (!unvisitedQueue.isEmpty()) {
//    DList::iterator it = unvisitedQueue.begin();
	  int * iterator = unvisitedQueue[0];
//    int min_node = it.getItem();
    int min_node = *iterator;

//    DList::iterator min = it; // remember iterator to minimum
    int * min = iterator; // remember iterator to minimum
    int min_distance = nodesH[min_node].distance;

//    while (!it.end()) {
    while ((iterator-unvisitedQueue[0] < unvisitedQueue.getHeapArraySize())) {
      if ((*iterator) < min_distance) {
        min_node = (*iterator);
        min = (iterator);
...
//      it.increment();
    ...
    unvisitedQueue.remove(*min); // remove min from queue
